...
#itemCountByCategory = grouped.agg(Count_Of_Items=pd.NamedAgg(column="ItemPrice", aggfunc="count"),)
...
# rename column
itemCountByCategory = itemCountByCategory.rename(columns={'ItemPrice': 'CountByCategory'}, inplace=True)
print(itemCountByCategory)
print ("--------------------------------------------")

plt.figure(figsize=(10, 6))
plt.subplot(1, 2, 1)
ax = sns.barplot(x='Category', y='CountByCategory', data=itemCountByCategory)
plt.xlabel("Category")
plt.ylabel("Count of Items in each Category")

plt.subplot(1, 2, 2)
plt.pie(itemCountByCategory['CountByCategory'], labels=itemCountByCategory.index, autopct='%1.2f%%')
...
# rename column
itemPriceByCategory = itemPriceByCategory.rename(columns={'ItemPrice': ' SumItemPrice'})
