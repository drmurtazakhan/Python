...
## age
age_binwidth = 10
age_bins = list(range(0, 90, age_binwidth))

plt.figure(figsize=(10, 6))
age_ax = sns.histplot(df[numerical[0]], kde=True,  binwidth=age_binwidth, stat=AggregateStat)

print('Age Distribution of Samples')
for bar, b0, b1 in zip(age_ax.containers[0], age_bins[:-1], age_bins[1:]):
    print(f'{b0:3d} - {b1:3d}: {bar.get_height():4.0f}')
## -------------------------------------------------------    
## bmi
bmi_binwidth = 10
bmi_bins = list(range(0, 60, bmi_binwidth))

plt.figure(figsize=(10, 6))
#sns.histplot(df[numerical[1]], kde=True, bins=noOfBins, stat=AggregateStat)
bmi_ax = sns.histplot(df[numerical[1]], kde=True, binwidth=bmi_binwidth, stat=AggregateStat)

print('BMI Distribution of Samples')
for bar, b0, b1 in zip(bmi_ax.containers[0], bmi_bins[:-1], bmi_bins[1:]):
    print(f'{b0:3d} - {b1:3d}: {bar.get_height():4.0f}')
